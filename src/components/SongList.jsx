...
// Google Sheets 곡목 시트 URL
const SHEETS_URL = `https://docs.google.com/spreadsheets/d/${CONFIG.SHEET_ID}/edit#gid=0`;
  ...
  return (
    <div>
      ...
      <div className="flex items-start justify-between mb-4 gap-3">
        <div>
          <h1 className="text-xl md:text-2xl font-bold">🎵 곡목 목록</h1>
          <p className={`text-xs md:text-sm mt-1 ${darkMode ? "text-gray-400" : "text-gray-500"}`}>
            ...
            {error && <span className="text-amber-400 ml-2">⚠ 데모 데이터</span>}
          </p>
        </div>
        {/* 구글 시트 버튼 */}
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="flex items-center gap-1.5 px-3 py-2 rounded-xl bg-green-500 text-white text-xs font-semibold hover:bg-green-600 transition-colors shadow-sm flex-shrink-0"
        >
          <span>📝</span>
          <span className="hidden sm:inline">곡목 추가</span>
          <span className="sm:hidden">추가</span>
        </a>
      </div>

      {/* Search */}
      <div className={`flex items-center rounded-xl border ${darkMode ? "bg-gray-800 border-gray-700" : "bg-white border-gray-200"} px-3 py-2 gap-2 mb-4`}>
      ...
      <div className="flex gap-2 mb-4 flex-wrap">
        {CATEGORIES.map(cat => {
          const count = cat === "전체" ? songs.length : songs.filter(s => s[cat] === "O").length;
          return (
            <button
              key={cat}
            ...
            >
              {cat}
              <span className={`ml-1 text-xs ${category === cat ? "text-amber-100" : darkMode ? "text-gray-500" : "text-gray-400"}`}>
                {count}
              </span>
            </button>
          );
      ...
      {loading ? (
        ...
        <>
          {/* 데스크탑: 테이블 */}
          <div className={`hidden md:block rounded-2xl border overflow-hidden ${darkMode ? "border-gray-800" : "border-gray-200"} shadow-sm`}>
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  filtered.map((song, i) => {
                    ...
                    return (
                      <tr key={i} className={`border-t transition-colors ${darkMode ? "border-gray-800 hover:bg-gray-800/50" : "border-gray-100 hover:bg-amber-50/80"}`}>
                        <td className={`px-4 py-3 font-mono text-xs ${darkMode ? "text-gray-500" : "text-gray-400"}`}>{i + 1}</td>
                        <td className={`px-4 py-3 font-medium ${darkMode ? "text-white" : "text-gray-900"}`}>{song.제목}</td>
                        <td className="px-4 py-3">
                          <div className="flex gap-1.5 flex-wrap">
                            {cats.map(cat => (
                              <span key={cat} className={`px-2 py-0.5 rounded-full text-xs font-medium border ${tagMap[cat]}`}>
                                {cat}
                              </span>
                            ))}
                          </div>
            ...
            {filtered.length === 0 ? (
              ...
              filtered.map((song, i) => {
                ...
                return (
                  ...
                  >
                    <span className={`text-xs font-mono w-6 flex-shrink-0 ${darkMode ? "text-gray-500" : "text-gray-400"}`}>{i + 1}</span>
                    <span className={`flex-1 text-sm font-medium ${darkMode ? "text-white" : "text-gray-900"}`}>{song.제목}</span>
                    <div className="flex gap-1 flex-wrap justify-end">
                      {cats.map(cat => (
                        <span key={cat} className={`px-1.5 py-0.5 rounded-full text-xs font-medium border ${tagMap[cat]}`}>
